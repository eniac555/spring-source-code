...
public class A01 {
    ...
    public static void main(String[] args) throws NoSuchFieldException, IllegalAccessException, IOException {
        ...
        //context.getBean("aaa");//getBean方法继承自BeanFactory

        /*
            1. 到底什么是 BeanFactory
                - 它是 ApplicationContext 的父接口
                - 它才是 Spring 的核心容器, 主要的 ApplicationContext 实现都【组合】了它的功能
                【组合】就意味着: BeanFactory是ApplicationContext的一个成员变量
         */
        ...
        //继承自 MessageSource 接口
        System.out.println(context.getMessage("hi", null, Locale.CHINA));
        System.out.println(context.getMessage("hi", null, Locale.ENGLISH));
        System.out.println(context.getMessage("hi", null, Locale.JAPANESE));

        //继承自 ResourcePatternResolver 接口
        Resource[] resources = context.getResources("classpath*:META-INF/spring.factories");
        for (Resource resource : resources) {
            System.out.println(resource);
        }

        //继承自 EnvironmentCapable 接口
        System.out.println(context.getEnvironment().getProperty("java_home"));
        System.out.println(context.getEnvironment().getProperty("server.port"));

        //继承自 ApplicationEventPublisher 接口
        //context.publishEvent(new UserRegisteredEvent(context));//发事件
        context.getBean(Component1.class).register();//发事件
